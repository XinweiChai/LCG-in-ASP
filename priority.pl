...
%---------------
%priority of states
node(B,N) :- branch(B), nodeState(B,N,_,_).
prior(P1,S1,P2,S2) :- nodeState(B,N1,P1,S1), nodeState(B,N2,P2,S2), N1>N2. 
prior(P,S1,P,S2) :- nodeState(B1,N1,P,S1), nodeState(B2,N2,P,S2), init(P,S1), S1!=S2.
prior(P3,S3,P,S2) :- nodeState(B1,N1,P,S1), nodeState(B2,N2,P,S2), init(P,S1), S1!=S2, nodeState(B1,N1-1,P3,S3).
prior(P1,S1,P3,S3) :- prior(P1,S1,P2,S2), prior(P2,S2,P3,S3).
localState(P,S) :- nodeState(_,_,P,S).
1{seq(1..O,P,S)}1 :- localState(P,S), O={node(B,N):nodeState(B,N,_,_)}.
